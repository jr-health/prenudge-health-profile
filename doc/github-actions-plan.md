...
| 7 | Entscheidung 5 pinnt Sunburst/Browse auf den Commit des Release-Tags — aber welcher Commit bekommt den Tag? (2026-09-22) | **`release.yml` regeneriert `health-profile.json`/`render/browse.*.html`/`render/sunburst.*.html` und committet sie auf `main`, *bevor* der Tag/Release erstellt wird** (statt wie bisher: Tag zuerst, Regenerierung erst danach asynchron über `update-profile.yml`s `workflow_run`-Trigger). `target_commitish` wird dabei explizit auf den frischen Commit-SHA gesetzt, nicht dem Action-Default überlassen. Dafür entfällt der `push: tags:`-Trigger; Releases laufen nur noch über `workflow_dispatch`. | Bug gefunden 2026-09-22: Ein Tag zeigte auf den Commit des *vorherigen* `update-profile.yml`-Laufs, nicht auf einen Commit mit zur eigenen Version passenden Sunburst/Browse-Dateien — weil `update-profile.yml` immer erst *nach* Abschluss von `release.yml` läuft (`workflow_run`-Trigger), der Tag zu dem Zeitpunkt aber schon fix war. Sunburst/Browse zeigten dadurch strukturell **immer** die vorletzte statt die aktuelle Version, unabhängig von Entscheidung-5-Korrektheit. Mit direktem Tag-Push wäre der Tag-Commit schon vor Workflow-Start fixiert und könnte die Regenerierung nicht mehr vorher einschließen — daher nur noch `workflow_dispatch`. |
